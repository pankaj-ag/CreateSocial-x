    ...
    <form enctype="multipart/form-data" id="post-form" class="form-horizontal"  action="" method="post">
        ...
        <div class="editor-header">
            <ul class="nav clearfix">
                <li><a class="current status" href=""><i class="icon ion-edit text-primary"></i> {{trans('post.status')}}</a> </li>
                ...
                <li class=" fileupload fileupload-exists" data-provides="fileupload">
					...
					<a id="post-editor-photos-selector" title="{{trans('post.attach-photos')}}"  class="photo btn-file">
                        ...
                        <span class="fileupload-exists"><i class="icon ion-ios-photos-outline text-warning"></i> {{trans('post.add-photos')}}</span>
                ...
                <li>
                    <a class="content-type-toggle video" data-type="video" title="{{trans('post.share-video-from')}}" data-placeholder="{{trans('post.share-video-from')}}" href="">
                        <i class="icon ion-android-film text-default"></i> {{trans('post.share-videos')}}
                    </a>
                </li>

                <li>
                    <a href="" class="post-add-file-trigger"><i class="icon ion-android-upload text-success"></i> {{trans('post.share-file')}}</a>
                </li>
            ...
            <div id="post-type-content">
                ...
                <div id="post-editor-video-upload-container" class=" fileupload fileupload-exists" data-provides="fileupload">
					...
					<a class="btn-file btn-success" style="padding:0px 5px">
                        <span class="fileupload-new"><i class="icon ion-ios-cloud-upload-outline"></i> {{trans('post.upload-videos')}}</span>
                        <span class="fileupload-exists"><i class="icon ion-ios-cloud-upload-outline"></i> {{trans('post.upload-videos')}}</span>
                ...
                <li class="action-container">
                    ...
                    <div id="post-privacy-container" class="btn-group">
                        ...
                        @if (!isset($to) and !isset($offPrivacy))
                            <!--<button class="btn btn-wht btn-midi btn-xs dropdown-toggle" title="{{trans('post.who-see-this')}}" data-toggle="dropdown"><i class="icon ion-locked"></i> <span>{{Auth::user()->present()->postPrivacyName()}}</span> <i class="caret"></i></button>-->
							<button id="post-editor-privacy-selector" class="btn btn-wht btn-midi btn-xs dropdown-toggle" title="{{trans('post.who-see-this')}}" data-toggle="dropdown"><i class="icon ion-locked"></i> <span>{{Auth::user()->present()->postPrivacyName()}}</span> <i class="caret"></i></button>
                    ...
                    <button id="post-submit-button" class="btn btn-midi btn-xs btn-success"><i class="icon ion-compose"></i> {{trans('post.post')}}</button>
