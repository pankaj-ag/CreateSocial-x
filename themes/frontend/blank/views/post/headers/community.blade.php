<div class="post-header clearfix">
            ...
            <div class="header-body">
                <h3 class="title">
                    ...
                    <a data-ajaxify="true" href="{{$post->user->present()->url()}}">{{$post->user->fullname}}</a> <span> {{($post->shared) ? 'shared a post' : null}}</span>
                    {{Theme::section('user.verified', ['user' => $post->user])}}

                        @if(Request::segment(1) != 'community' and $post->community)
                            &#160;&#160;<i class="icon icon ion-arrow-right-b"></i>&#160;&#160;<a data-ajaxify="true" href="{{$post->community->present()->url()}}">{{$post->community->title}}</a>
                        @endif
                ...
                <div class="post-action-dropdown dropdown">
                    <a  data-click="dropdown" data-toggle="dropdown" class="dropdown-toggle" href=""><i class="icon ion-ios-arrow-down"></i></a>
