...
<div id="post-{{$post->id}}" {{$post->type}} {{$post->content_type}} {{$post->page_id}} {{$post->community_id}}  class="post-media user-post-{{$post->user->id}} clearfix">
    ...
    <div class="media original-poster">
        ...
        <div class="media-body">
            <h5 class="media-heading">{{$post->sharedUser->fullname}} {{trans('post.originally-post-this')}}</h5>
        </div>
    </div>
    @endif
    ...
    <div class="post-footer">
        <ul class="nav nav-left">
            <li>
                ...
                <a data-is-login="{{Auth::check()}}" data-status="{{($hasLike) ? '1' : 0}}" class="like-button" data-like="{{trans('like.like')}}" data-unlike="{{trans('like.unlike')}}" data-id="{{$post->id}}" data-type="post" href=""> <span>{{($hasLike) ? trans('like.unlike') : trans('like.like')}}</span></a>
            </li>
            @if($post->content_type != 'auto-post')
            <li><a href="" data-is-login="{{Auth::check()}}" class="post-share-button" data-id="{{$post->id}}"><span>{{trans('post.share')}}</span></a> </li>
            @endif
            @if (Auth::check())
            <li>
        ...
        <ul class="nav nav-right pull-right">
            @if($post->content_type != 'auto-post')
            <li><a data-id="{{$post->id}}" class="post-activity-loader" href="{{URL::route('load-shares', ['id' => $post->id])}}"><i class="icon ion-reply"></i> <span class="post-share-count-{{$post->id}}">{{$post->shared_count}}</span></a> </li>
            @endif
            <li><a data-id="{{$post->id}}" class="post-activity-loader" href="{{URL::route('show-likes', ['post' => 'post', 'id' => $post->id])}}"><i class="icon ion-ios-heart"></i> <span class="post-like-count-{{$post->id}}">{{$post->countLikes()}}</span></a> </li>
