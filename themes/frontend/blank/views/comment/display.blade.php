...
<div id="reply-{{$comment->id}}" class="reply media reply-{{$comment->id}}">
    ...
    <div class="media-body">
        ...
        @endif
		<span class="post-time"> <span title="{{$comment->present()->time()}}">{{formatDTime($comment->created_at)}}</span></span>
        ...
        <!--<a href="">Like</a>-->
        @if ($comment->present()->canDelete())
            <a data-comment-id="{{$comment->id}}" class="comment-edit-button pull-right" href=""><i class="icon ion-edit"></i> {{trans('global.edit')}}</a>
        @endif	
    </div>
</div>
@else
