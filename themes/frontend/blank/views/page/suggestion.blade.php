        ...
        <div class=" user media media">
            ...
            <div class="media-body">
                ...
                <p>
					...
					<span class="pull-right mr"><?php $hasLike = $page->hasLiked()?>
                    <a  data-is-login="{{Auth::check()}}" data-status="{{($hasLike) ? '1' : 0}}" class="btn btn-default btn-xs like-button" data-like="{{trans('like.like')}}" data-unlike="{{trans('like.unlike')}}" data-id="{{$page->id}}" data-type="page" href=""><i class="icon ion-thumbsup"></i> <span>{{($hasLike) ? trans('like.unlike') : trans('like.like')}}</span></a></span>
                </p>
            </div>
        </div>
